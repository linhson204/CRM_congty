...
export default function TableHisUsePointEmp() {
  const router = useRouter();
  const { com_id, userName, idQLC } = decodeToken();
  const [formData, setFormData] = useState<any>({});
  const [historyList, setHistoryList] = useState([]);
  const { setHeaderTitle, setShowBackButton, setCurrentPath }: any =
    useHeader();
  useEffect(() => {
    setHeaderTitle(`Lịch sử sử dụng điểm của ${userName}`);
  ...
  const getHistoryAddPoint = () => {
    axiosCRMv2("/customer/HistoryCrmOfEm", {
      emp_id: idQLC,
