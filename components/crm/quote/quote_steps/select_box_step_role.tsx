...
import RoleDropDownDataStep from "./select_box_dropdown_data_role";
export default function RoleSelectBoxStep({
  title = "",
  value = "Chọn",
  placeholder = "",
  data = [],
  ...
  const { quote_role } = useContext(QuoteContext)
  ...
  useEffect(() => {
    if (roleValue !== 0) {
      setValue(roleValue)
      setRoleLabel(quote_role.find(item => item.value === roleValue).label)
    }
  }, [roleValue])
      ...
      >
        <span className={`${styles.selection}`}>
          ...
          >
            ...
            >
              {roleValue === 0 && roleLabel === '' ? value : roleLabel}
            </span>
            <span
        ...
        {isOpen && <RoleDropDownDataStep data={quote_role} value={value} setValue={setRoleValue} />}
      </span>
    </div>
  );
}
