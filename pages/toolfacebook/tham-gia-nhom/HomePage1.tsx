...
export default function DangBai() {
  ...
  useEffect(() => {
  const fetchData = async () => {
    const res = await fetch('../../pages/api/account.json'); // lưu ý: đường dẫn phải từ /public/
  ...
  function countGrJoin(users) {
    const joined = users.groups.filter(group => group.isJoin === true).length;
    const notjoined = users.groups.filter(group => group.isJoin === false).length;
