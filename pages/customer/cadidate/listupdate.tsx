...
export default function CadidateList() {
  ...
  useEffect(() => {
    const fetchAPi = async () => {
        ...
        let res = await axios.post("https://api.timviec365.vn/api/timviec/admin/uv/list/update3",
         formData,
         {  headers: { Authorization: `Bearer ${accessToken}` } }
         );
        console.log(res.data.data.data.list)
        setList(res.data.data.data.list)
        res = await axios.post("https://api.timviec365.vn/api/timviec/admin/uv/list/count_candi_update3",
        formData,
        {  headers: { Authorization: `Bearer ${accessToken}` } }
        );
  ...
  const columns: ColumnsType<TypePointTable> = [
    ...
    },
    {
      title: "Tên ứng viên",
      key: "use_first_name",
      dataIndex: "use_first_name",
      render(value, record, index) {
        return (
          <p className={styles.text}>
            {record.use_first_name}
          </p>
        );
      },
    },
    ...
    },
    {
      ...
      render(value, record, index) {
        if (record.beseen) {
          return <p>{record.use_phone_tk || record.use_phone}</p>;
        } else {
          return (
            <button
              style={{ color: "red" }}
              onClick={() => usePoint(record.use_id)}
    ...
    <div ref={mainRef} className={styleHome.main}>
      ...
      >
        ...
        />
          ...
          name="use_phone"
          className={styles.input}
          onChange = {(e)=> handleChangeSearch(e,"use_phone")}
        />
          <input
          placeholder="Nhập số điện thoại đăng ký"
          name="use_phone_tk"
          className={styles.input}
          onChange = {(e)=> handleChangeSearch(e,"use_phone_tk")}
        />
                <input
          placeholder="Nhập mail"
          name="use_email"
          className={styles.input}
          onChange = {(e)=> handleChangeSearch(e,"use_email")}
        />
         <input
          placeholder="Nhập tên"
          name="use_first_name"
          className={styles.input}
          onChange = {(e)=> handleChangeSearch(e,"use_first_name")}
        />
        ...
        <div className={styles.element}>
          ...
          <input
            ...
            onChange = {(e)=> handleChangeSearch(e,"time_start")}
          />
        </div>
        <div className={styles.element}>
          <div>
          ...
          <input
            ...
            onChange = {(e)=> handleChangeSearch(e,"time_end")}
          />
        </div>
        {/* <div className={styles.element}>
          <div>
