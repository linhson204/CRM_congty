...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Lịch sử điểm - Work247");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Lịch sử điểm - Work247</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://viecday365.com/admin/modules/tbl_point/listing_ls.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
