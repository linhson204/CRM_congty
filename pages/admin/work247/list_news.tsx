...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Danh sách tin đăng - Work247");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Danh sách tin đăng - Work247</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://viecday365.com/admin/modules/newtd_kd/list_byuser.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
