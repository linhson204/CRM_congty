...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Đăng tin mới - Work247");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Đăng tin mới - Work247</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://viecday365.com/admin/modules/newtd/add.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
