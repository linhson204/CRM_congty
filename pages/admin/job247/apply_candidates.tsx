...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Ứng viên tự ứng tuyển - Job247");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Ứng viên tự ứng tuyển - Job247</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://job247.vn/admin2/gui-ung-vien/ung-vien-ung-tuyen?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
