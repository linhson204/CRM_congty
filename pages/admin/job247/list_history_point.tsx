...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Lịch sử điểm - Job247");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Lịch sử điểm - Job247</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://job247.vn/admin2/truong-diem/lich-su?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
