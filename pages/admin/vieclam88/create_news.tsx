...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Đăng tin mới - Vieclam88");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Đăng tin mới - Vieclam88</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://vieclam88.vn/admin/modules/newtd_kd/add.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
