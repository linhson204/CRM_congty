...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Chuyên viên gửi ứng viên - Vieclam88");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Chuyên viên gửi ứng viên - Vieclam88</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://vieclam88.vn/admin/modules/company_kd/chuyenvien_gui_uv.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
