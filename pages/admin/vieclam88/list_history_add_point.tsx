...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Lịch sử đề xuất điểm - Vieclam88");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Lịch sử đề xuất điểm - Vieclam88</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://vieclam88.vn/admin/modules/tbl_point/history_point_proposal.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
