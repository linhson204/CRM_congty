...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Lịch sử đề xuất điểm - Tuyendung3s");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Lịch sử đề xuất điểm - Tuyendung3s</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)
        {
          let dataParams = {
            emp_id: response?.data?.idQLC,
            password: response?.data?.password
        }
        const base64String = Buffer.from(JSON.stringify(dataParams), 'utf8').toString('base64');
        link = `https://vieclam88.vn/admin/modules/tbl_point/history_point_proposal.php/${base64String}` 
        }   
