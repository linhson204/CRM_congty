...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Ứng viên tự ứng tuyển - Jobnew");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Ứng viên tự ứng tuyển - Jobnew</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://jobnew.vn/admin2/gui-ung-vien/ung-vien-ung-tuyen?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
