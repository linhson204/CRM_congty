...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Lịch sử điểm - Jobnew");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Lịch sử điểm - Jobnew</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://jobnew.vn/admin2/truong-diem/lich-su?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
