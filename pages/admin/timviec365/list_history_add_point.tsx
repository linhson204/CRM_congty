...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Lịch sử cộng điểm - Timviec365");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Lịch sử cộng điểm - Timviec365</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://timviec365.vn/admin/modules/tbl_point/listing_plus.php?idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
