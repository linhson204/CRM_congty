...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Ứng viên tự ứng tuyển - Timviec365");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Ứng viên tự ứng tuyển - Timviec365</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://timviec365.vn/admin/modules/ungvien/uvcv_ntd_uv_tu_ung_tuyen.php?idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
