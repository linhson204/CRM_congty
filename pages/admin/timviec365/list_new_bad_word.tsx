...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Danh sách tin đăng chứa từ khóa nhạy cảm - Timviec365");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Danh sách tin đăng - Timviec365</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        const response = await axios.post(`https://api.timviec365.vn/api/qlc/employee/getInfoByToken`, {
        }, {
            headers: {
        ...
        if(response?.data?.idQLC)   link = `https://timviec365.vn/admin/modules/newtd/listing_keyhot.php?idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
