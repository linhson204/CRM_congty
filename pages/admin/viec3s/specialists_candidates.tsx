...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Chuyên viên gửi ứng viên - Viec3s");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Chuyên viên gửi ứng viên - Viec3s</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://viec3s.com/admin2/gui-ung-vien/chuyen-vien-gui-ung-vien?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
