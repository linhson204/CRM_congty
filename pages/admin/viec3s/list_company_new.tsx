...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("NTD đăng ký mới - Viec3s");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>NTD đăng ký mới - Viec3s</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://viec3s.com/admin2/nha-tuyen-dung/NTD-dang-ky-moi?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
