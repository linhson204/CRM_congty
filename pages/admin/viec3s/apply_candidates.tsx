...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Ứng viên tự ứng tuyển - Viec3s");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Ứng viên tự ứng tuyển - Viec3s</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
        ...
        if(response?.data?.idQLC)   link = `https://viec3s.com/admin2/gui-ung-vien/ung-vien-ung-tuyen?isModule=1&idQLC=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
