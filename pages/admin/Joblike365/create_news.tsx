...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Đăng tin mới - Joblike365");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Đăng tin mới - Joblike365</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://joblike365.com/admin/modules/tintuyendung/add.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
