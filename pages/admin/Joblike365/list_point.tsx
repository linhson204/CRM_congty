...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("Danh sách điểm - Joblike365");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>Danh sách điểm - Joblike365</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://joblike365.com/admin/modules/truongdiem/listing.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
