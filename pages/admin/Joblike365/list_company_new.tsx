...
function Home({link}) {
    ...
    useEffect(() => {
      setHeaderTitle("NTD đăng ký mới - Joblike365");
    }, [setHeaderTitle, , ]);
 ...
 if(link) return (
    <>
      <Head>
        ...
        <title>NTD đăng ký mới - Joblike365</title>
        <meta
          name="description"
...
export async function getServerSideProps(context: any) {
    ...
    try {
      ...
      if(response?.data?.idQLC)   link = `https://joblike365.com/admin/modules/nhatuyendung/listing_regis.php?emp_id=${response?.data?.idQLC}&password=${response?.data?.password}` 
    } catch (error: any) {
